...
```yaml
sources:
    databricks_prod_source:
        database_type: oracle
        kafka_topic_prefix: prod.prod_source
        # Unity Catalog format: catalog.schema.table_prefix
        iceberg_namespace: prod_catalog.bronze.prod_source
        ...
        database_connection:
            ...
            password: ${ORACLE_PROD_SOURCE_PASSWORD} # From Databricks Secrets
            schema: WALDO
...
ORACLE_PROD_SOURCE_PASSWORD=<password>
```
...
# Add secrets
databricks secrets put --scope prod_etl --key oracle_prod_source_password
...
```yaml
password: ${ORACLE_PROD_SOURCE_PASSWORD}
```

The `SecretsManager` class will:

1. Try to fetch from Databricks Secrets (scope: `etl`, key: `oracle_prod_source_password`)
2. Fall back to environment variable `ORACLE_PROD_SOURCE_PASSWORD`
...
```python
%run /Workspace/Repos/your-repo/jobs/direct_bulk_load.py --source databricks_prod_source
```
...
2. **Configure Task**:
    ...
    Parameters: ["--source", "databricks_prod_source"]
    ```
...
2. Configure job with:
    ...
    Parameters: ["--source", "databricks_prod_source", "--batch-size", "1000"]
...
1. **Table Maintenance**:
    ...
    ```sql
    -- Compact small files
    OPTIMIZE prod_catalog.bronze.prod_source.large_table;

    -- Remove old snapshots
    CALL prod_catalog.system.expire_snapshots('bronze.prod_source.large_table', TIMESTAMP '2024-01-01 00:00:00');
    ```
...
sources:
    local_minio:
        database_type: oracle
        iceberg_namespace: local.bronze.mydb
...
mc cp local/iceberg-warehouse/dev/bronze/mydb/metadata/snap-*.avro ./
```
